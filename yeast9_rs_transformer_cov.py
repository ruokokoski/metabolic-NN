...
def precompute_flux_correlation(data_path, output_cols, device="cpu"):
    """
    Compute the normalized covariance (correlation) matrix for outputs (fluxes) 
    and return it as a PyTorch tensor.

    Args:
        data_path (str): Path to CSV data.
        output_cols (list): Columns corresponding to fluxes (outputs).
        device (str or torch.device): Where to store the tensor.

    Returns:
        corr_tensor (torch.Tensor): Correlation matrix [num_fluxes, num_fluxes] on the device.
    """
    df = pd.read_csv(data_path)

    flux_data = df[[f"{col}_flux" for col in output_cols]].to_numpy(dtype=np.float32)
    flux_centered = flux_data - flux_data.mean(axis=0)

    # Covariance matrix
    cov_matrix = np.cov(flux_centered, rowvar=False)
...
def train_model(
    ...
    K = 256  # number of fixed eval outputs
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        for batch_X, batch_y in train_loader:
            ...
            # Randomly sample subset of outputs using covariance/uniform logic
