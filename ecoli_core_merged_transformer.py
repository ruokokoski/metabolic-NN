...
def train_model(
    ...
    model = FluxTransformer(
        ...
        d_ff=d_ff,
        dropout=dropout,
        input_length=input_length  # <-- K (inputs occupy first K token slots)
    ).to(device)
...
def plot_pre_attention_embeddings_tsne(model, output_cols_true, K, perplexity=30.0, save_path=None):
    device = next(model.parameters()).device
    n_out = len(output_cols_true)

    with torch.no_grad():
        token_ids = torch.arange(K, K + n_out, device=device)  # K..V-1
        emb = model.input_embedding(token_ids).cpu().numpy()

    embeddings_2d = TSNE(n_components=2, perplexity=perplexity, random_state=42).fit_transform(emb)
    plt.figure(figsize=(14, 10))
    plt.scatter(embeddings_2d[:, 0], embeddings_2d[:, 1], s=100, alpha=1.0)

    for i, flux in enumerate(output_cols_true):
        plt.annotate(flux.replace('_flux',''), (embeddings_2d[i,0], embeddings_2d[i,1]),
...
def evaluate_metrics(model, X_test, y_test, output_cols_true, K, device='cpu'):
    model = model.to(device).eval()

    with torch.no_grad():
        preds_all_layers, selected = model(X_test.unsqueeze(-1).to(device), output_subset=None)
        pred_last = preds_all_layers[:, :, -1]  # (N, V)

    y_pred = pred_last[:, K:].cpu().numpy()              # (N, V-K)
    y_true = y_test[:, K:].cpu().numpy()                 # (N, V-K)

    metrics = []
    for i, label in enumerate(output_cols_true):
...
if __name__ == "__main__":
    ...
    output_cols_true = outputs_ordered[K:]
