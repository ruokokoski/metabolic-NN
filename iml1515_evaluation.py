"""Shared evaluation helpers for iML1515 sampling-study models; no training runs on import."""
...
def input_contract(model_family="AB_union", exclude_cbl1=False):
    """Return the generator's ordered inputs; C also supports legacy 40-input runs."""
    ...
    if model_family in ("D", "E"):
        ...
        return import_module(f"generate_ecoli_iML1515_{model_family}_data").build_input_columns()
    raise ValueError(f"Unknown model family: {model_family}")


def load_reservoir(path, device, metadata_path=None, model_family="AB_union"):
    ...
    if inputs != input_contract(model_family, exclude_cbl1="EX_cbl1_e" not in inputs):
        raise ValueError(f"Checkpoint input order does not match model {model_family} generator.")
...
def load_amn(root, input_names=None, model_family="AB_union", feature_order="carbons_first"):
    ...
    fixed = {name: 0.0 for name in input_names}
...
def simulated_fidelity(reservoir, inputs, outputs, specs, xml_path, destination,
    ...
    for domain, spec in specs.items():
        ...
        for frame in pd.read_csv(spec["path"], chunksize=128):
            ...
            if any(name in frame and frame[name].ne(0).any() for name in forbidden):
            ...
            for name in set(required):
                ...
                if domain == "B" and name in MINN_CONTEXT_EXCHANGES[2:]:
                    lower[:, pos], upper[:, pos] = 0, frame[name].to_numpy(float)
                else:
                    lower[:, pos] = -frame[name].to_numpy(float)
            prediction = []
            with torch.no_grad():
