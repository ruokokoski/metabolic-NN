...
- `generate_ecoli_iML1515_MINN_data_tazza.py` is the Tazza-style MINN
  reservoir ablation: it samples glucose/oxygen uptake caps and
  CO2/ethanol/acetate secretion caps from widened integer envelopes around the
  observed Ishii ranges, while retaining iML1515, pFBA, and the large
  FluxTransformer training-data scale.
- AMN-style iML1515 simulated media data should use
  `generate_ecoli_iML1515_AMN_data.py` for new runs.
...
- The AMN generator keeps the Faure-style 38-exchange nutrient identity
...
- The AMN medium sampling follows Faure, including fixed glycerol and amino-acid
  caps of `2.2`, with one deliberate sampling difference: oxygen is flexible.
  The generator samples `EX_o2_e` unless `--fixed-oxygen` is explicitly used
  for an ablation. Document any change to the `2.2` caps in the AMN notes and
  thesis notes.
- `generate_ecoli_iML1515_AMN_MINN_data.py` is the shared-reservoir generator
  ...
  separate `minn`, `faure`, and `mixed` regimes. Its MINN/mixed rows use
  independently sampled integer uptake/secretion caps, and all regimes use a
  shared non-carbon base nutrient rate of 50. In its `faure` rows, oxygen is the
  only extra randomized AMN medium variable relative to Faure; the fixed rate of
  50 is cross-regime normalization rather than an additional sampled variable.
  Current defaults generate one million training samples with seed 42 and a
  training-data output prefix; manually override those CLI options for the
  seed-9 test set. Do not present the overall shared generator as an exact Faure
  setup.
