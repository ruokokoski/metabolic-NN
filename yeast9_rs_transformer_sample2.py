...
def train_model(
    ...
    df_outputs = pd.read_csv(
        DATA_PATH,
        usecols=[f"{o}_flux" for o in outputs],
        dtype=np.float32
    )
    output_matrix = df_outputs.to_numpy()
...
if __name__ == "__main__":
    ...
    epsilon = 0.5
    temperature = 5.0
    model_name = f"yeast9_d{d_model}_h{n_heads}_l{n_layers}_ff{d_ff}_sample{str(output_sample_ratio).replace('.', '_')}_eps{str(epsilon).replace('.', '_')}"
    ...
    train_loss, test_loss, model, optimizer = train_model(d_model, n_heads, n_layers, d_ff, num_epochs, learning_rate, dropout, model_name, output_sample_ratio, epsilon)
