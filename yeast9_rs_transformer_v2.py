...
def train_model(
    ...
    # ------------------------------------------------------------
    # Fixed evaluation subset (global indices)
    # ------------------------------------------------------------
    K = 512
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        for batch_X, batch_y in train_loader:
            ...
            # ----------------------------------------------------
            # Sample output subset (global indices)
            # ----------------------------------------------------
            if output_sample_ratio >= 1.0:
                sampled_global = torch.arange(
                    output_start_idx, output_start_idx + total_outputs,
                ...
                sampled_global = sampled_rel + output_start_idx

            # track sampling distribution
            sample_counter.update(sampled_global.cpu().tolist())

            # ----------------------------------------------------
            # SPEED-UP: forward only over input indices + sampled outputs
            # ----------------------------------------------------
            predictions, selected_indices = model(
                batch_X,
                output_subset=sampled_global
            )
        ...
        with torch.no_grad():
            for batch_X, batch_y in test_loader:
                ...
                # partial forward (inputs + fixed eval outputs)
                predictions, selected_indices = model(
                    batch_X,
