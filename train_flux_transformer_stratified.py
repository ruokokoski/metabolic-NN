...
MODEL_NAME = "ecoli_core_1M_stratloss"
...
def create_dataloaders(
    ...
    if train_sample_weights is not None:
        ...
        if train_sample_weights.shape != (len(train_indices),):
            raise ValueError(
                "train_sample_weights must have shape "
                f"({len(train_indices)},), got {train_sample_weights.shape}"
            )
...
def compute_flux_loss_scales(
    ...
    *,
    ...
    """
    Estimate one loss scale per output flux from the training split only.

    The default uses a robust 95%-5% range, then falls back to standard
    deviation for sparse fluxes whose central quantile range is nearly zero.
    """
    train_indices = np.asarray(train_indices, dtype=np.int64)
    if train_indices.size == 0:
        raise ValueError("train_indices is empty; cannot compute loss scales.")

    if max_samples is not None and max_samples > 0 and train_indices.size > max_samples:
        rng = np.random.default_rng(random_state)
        scale_indices = rng.choice(train_indices, size=int(max_samples), replace=False)
    else:
        scale_indices = train_indices

    scale_indices = np.sort(np.asarray(scale_indices, dtype=np.int64))
    y_sample = np.asarray(y[scale_indices], dtype=np.float32)

    method = str(method).lower()
    if method in {"q95_q05", "quantile_range", "robust_range"}:
        lo, hi = np.nanquantile(y_sample, [q_low, q_high], axis=0)
        scales = hi - lo
        std = np.nanstd(y_sample, axis=0)
        scales = np.where(np.isfinite(scales) & (scales >= min_scale), scales, std)
    elif method in {"std", "standard_deviation"}:
        scales = np.nanstd(y_sample, axis=0)
    elif method == "iqr":
        q25, q75 = np.nanquantile(y_sample, [0.25, 0.75], axis=0)
        scales = q75 - q25
        std = np.nanstd(y_sample, axis=0)
        scales = np.where(np.isfinite(scales) & (scales >= min_scale), scales, std)
    else:
        raise ValueError(f"Unknown loss scale method: {method!r}")

    scales = np.asarray(scales, dtype=np.float32)
    scales = np.where(np.isfinite(scales) & (scales >= min_scale), scales, float(min_scale))

    print(
        "Per-flux normalized loss scales: "
        f"method={method}, samples={len(scale_indices):,}, "
        f"min={float(np.min(scales)):.4g}, "
        f"median={float(np.median(scales)):.4g}, "
        f"max={float(np.max(scales)):.4g}"
    )
    return scales


def _flux_token_name(name):
    return name if name.endswith("_flux") else f"{name}_flux"


def resolve_flux_indices(outputs, names, *, label):
    index_by_name = {name: idx for idx, name in enumerate(outputs)}
    indices = []
    missing = []
    for name in names:
        token = _flux_token_name(name)
        if token in index_by_name:
            indices.append(index_by_name[token])
        else:
            missing.append(token)

    if missing:
        print(f"Warning: missing {label} flux tokens: {', '.join(missing)}")
    if indices:
        resolved = ", ".join(outputs[i] for i in indices)
        print(f"{label} fluxes ({len(indices)}): {resolved}")
    else:
        print(f"{label} fluxes: none resolved")
    return indices


def _subset_y_by_columns(y, row_indices, col_indices):
    row_indices = np.asarray(row_indices, dtype=np.int64)
    col_indices = list(col_indices)
    subset = np.empty((len(row_indices), len(col_indices)), dtype=np.float32)
    for out_col, y_col in enumerate(col_indices):
        subset[:, out_col] = np.asarray(y[row_indices, int(y_col)], dtype=np.float32)
    return subset


def compute_hard_flux_training_stats(
    y,
    train_indices,
    outputs,
    *,
    ...
    tail_quantile=0.90,
    extreme_tail_quantile=0.97,
    min_tail_threshold=1.0,
    sparse_active_boost=1.5,
    high_tail_boost=1.5,
    extreme_tail_boost=2.5,
    max_sample_weight=5.0,
):
    """
    Build targeted training statistics for difficult fluxes.

    The returned sample weights are in train_indices order and are intended only
    for the training loader. Validation/test distributions remain unchanged.
    """
    train_indices = np.asarray(train_indices, dtype=np.int64)
    if train_indices.size == 0:
        raise ValueError("train_indices is empty; cannot compute hard-flux stats.")
    if not (0.0 < tail_quantile < 1.0):
        raise ValueError("tail_quantile must be between 0 and 1.")
    if not (0.0 < extreme_tail_quantile < 1.0):
        raise ValueError("extreme_tail_quantile must be between 0 and 1.")
    if extreme_tail_quantile < tail_quantile:
        raise ValueError("extreme_tail_quantile must be >= tail_quantile.")

    hard_indices = resolve_flux_indices(outputs, hard_flux_names, label="Hard")
    sparse_indices = resolve_flux_indices(outputs, sparse_flux_names, label="Sparse")
    tail_indices = resolve_flux_indices(outputs, tail_flux_names, label="Tail")

    all_target_indices = sorted(set(hard_indices) | set(sparse_indices) | set(tail_indices))
    sample_weights = np.ones(len(train_indices), dtype=np.float32)
    hard_flux_weights = np.zeros(len(outputs), dtype=np.float32)
    tail_thresholds = np.full(len(outputs), 1.0e30, dtype=np.float32)

    if not all_target_indices:
        print("No hard fluxes resolved; using uniform training samples and base loss only.")
        return {
            "sample_weights": sample_weights,
            "hard_flux_weights": hard_flux_weights,
            "tail_thresholds": tail_thresholds,
            "hard_flux_indices": hard_indices,
            "sparse_flux_indices": sparse_indices,
            "tail_flux_indices": tail_indices,
        }

    y_subset = _subset_y_by_columns(y, train_indices, all_target_indices)
    local_col_by_global = {idx: local for local, idx in enumerate(all_target_indices)}
    hard_flux_weights[hard_indices] = 1.0

    for idx in sparse_indices:
        values = y_subset[:, local_col_by_global[idx]]
        active = np.abs(values) > float(activity_threshold)
        sample_weights[active] += float(sparse_active_boost)
        print(
            f"Sparse stratification {outputs[idx]}: "
            f"active_frac={float(active.mean()):.4g}, boost={sparse_active_boost:g}"
        )

    for idx in tail_indices:
        values = y_subset[:, local_col_by_global[idx]]
        abs_values = np.abs(values)
        active_abs = abs_values[abs_values > float(activity_threshold)]
        if active_abs.size == 0:
            threshold = 1.0e30
            extreme_threshold = 1.0e30
        else:
            threshold = float(np.nanquantile(active_abs, tail_quantile))
            extreme_threshold = float(np.nanquantile(active_abs, extreme_tail_quantile))
            if not np.isfinite(threshold) or threshold < min_tail_threshold:
                threshold = float(min_tail_threshold)
            if not np.isfinite(extreme_threshold) or extreme_threshold < threshold:
                extreme_threshold = threshold

        tail_thresholds[idx] = threshold
        high_tail = abs_values >= threshold
        extreme_tail = abs_values >= extreme_threshold
        sample_weights[high_tail] += float(high_tail_boost)
        sample_weights[extreme_tail] += float(extreme_tail_boost)
        print(
            f"Tail stratification {outputs[idx]}: "
            f"q{tail_quantile:.2f}={threshold:.4g}, "
            f"q{extreme_tail_quantile:.2f}={extreme_threshold:.4g}, "
            f"high_frac={float(high_tail.mean()):.4g}, "
            f"extreme_frac={float(extreme_tail.mean()):.4g}"
        )

    sample_weights = np.clip(sample_weights, 1.0, float(max_sample_weight))
    print(
        "Stratified sample weights: "
        f"min={float(sample_weights.min()):.3g}, "
        f"median={float(np.median(sample_weights)):.3g}, "
        f"mean={float(sample_weights.mean()):.3g}, "
        f"max={float(sample_weights.max()):.3g}, "
        f"weighted_samples={int(np.sum(sample_weights > 1.0)):,}/{len(sample_weights):,}"
    )
...
def train_model(
    ...
    hard_flux_weights=None,
    hard_flux_extra_weight=0.25,
    tail_underprediction_weight=0.75,
):
    ...
    regression_criterion = nn.HuberLoss(reduction="none")

    if loss_scales is None:
        loss_scale_tensor = torch.ones(1, vocab_size, 1, dtype=torch.float32, device=device)
        loss_scale_name = "none"
    else:
        loss_scales = np.asarray(loss_scales, dtype=np.float32)
        if loss_scales.shape != (vocab_size,):
            raise ValueError(
                f"loss_scales must have shape ({vocab_size},), got {loss_scales.shape}"
            )
        if not np.all(np.isfinite(loss_scales)) or np.any(loss_scales <= 0):
            raise ValueError("loss_scales must contain finite positive values.")
        loss_scale_tensor = torch.as_tensor(
            loss_scales, dtype=torch.float32, device=device
        ).view(1, vocab_size, 1)
    ...
    hard_flux_weight_tensor = _as_loss_tensor(hard_flux_weights, 0.0, "hard_flux_weights")
    tail_threshold_tensor = _as_loss_tensor(tail_thresholds, 1.0e30, "tail_thresholds")

    print(
        "Targeted hard-flux loss: "
        f"hard_flux_extra_weight={hard_flux_extra_weight:g}, "
        f"tail_underprediction_weight={tail_underprediction_weight:g}"
    )

    def _combined_loss(pred_last, target, selected_indices):
        selected_scales = loss_scale_tensor[:, selected_indices, :]
        selected_hard_weights = hard_flux_weight_tensor[:, selected_indices, :]
        selected_tail_thresholds = tail_threshold_tensor[:, selected_indices, :]

        abs_target = target.abs()
        high_tail_mask = abs_target >= selected_tail_thresholds
        underpredicts_positive = (target >= 0) & (pred_last < target)
        underpredicts_negative = (target < 0) & (pred_last > target)
        tail_underprediction_mask = (
            high_tail_mask & (underpredicts_positive | underpredicts_negative)
        ).float()
        ...
        regression_weights = (
            1.0
            + float(hard_flux_extra_weight) * selected_hard_weights
            + float(tail_underprediction_weight) * tail_underprediction_mask
        )
        return (regression_loss * regression_weights).mean()
    ...
    if start_epoch >= num_epochs:
        ...
        train_meta = {
            ...
            "hard_flux_extra_weight": float(hard_flux_extra_weight),
            "tail_underprediction_weight": float(tail_underprediction_weight),
        }
        _remove_file_if_exists(best_checkpoint_path)
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        for batch_idx, (batch_X, batch_y) in enumerate(train_loader):
            ...
            loss = _combined_loss(pred_last, target, selected_indices)
            loss.backward()
...
if __name__ == "__main__":
    ...
    try:
        ...
        loss_scales = compute_flux_loss_scales(
            y,
            train_indices,
            method=loss_scale_method,
            min_scale=loss_scale_min,
            max_samples=loss_scale_max_samples,
            random_state=42,
        )
        hard_flux_stats = compute_hard_flux_training_stats(
            y,
            train_indices,
            outputs,
            hard_flux_names=hard_flux_names,
            sparse_flux_names=sparse_flux_names,
            tail_flux_names=tail_flux_names,
            activity_threshold=activity_threshold,
            tail_quantile=tail_quantile,
            extreme_tail_quantile=extreme_tail_quantile,
            min_tail_threshold=tail_min_threshold,
            sparse_active_boost=sparse_active_boost,
            high_tail_boost=high_tail_boost,
            extreme_tail_boost=extreme_tail_boost,
        ...
        train_loader, test_loader = create_dataloaders(
            ...
            device=device,
            num_workers=0,
            train_sample_weights=hard_flux_stats["sample_weights"],
        )
        ...
        train_loss, test_loss, model, optimizer, train_meta = train_model(
            ...
            tail_thresholds=hard_flux_stats["tail_thresholds"],
            hard_flux_weights=hard_flux_stats["hard_flux_weights"],
            hard_flux_extra_weight=hard_flux_extra_weight,
            tail_underprediction_weight=tail_underprediction_weight,
        )
        ...
        checkpoint = {
            ...
            "config": {
                ...
                "use_activity_head": False,
                "use_stratified_sampler": True,
                "activity_threshold": activity_threshold,
                "hard_flux_names": list(hard_flux_names),
                "sparse_flux_names": list(sparse_flux_names),
                "tail_flux_names": list(tail_flux_names),
                "tail_quantile": tail_quantile,
                "extreme_tail_quantile": extreme_tail_quantile,
            ...
            "data_info": {
                ...
                "hard_flux_indices": [int(i) for i in hard_flux_stats["hard_flux_indices"]],
                "sparse_flux_indices": [int(i) for i in hard_flux_stats["sparse_flux_indices"]],
                "tail_flux_indices": [int(i) for i in hard_flux_stats["tail_flux_indices"]],
                "hard_flux_weights": [float(x) for x in hard_flux_stats["hard_flux_weights"]],
                "tail_thresholds": [float(x) for x in hard_flux_stats["tail_thresholds"]],
                "train_sample_weights_summary": {
                    "min": float(np.min(hard_flux_stats["sample_weights"])),
                    "median": float(np.median(hard_flux_stats["sample_weights"])),
                    "mean": float(np.mean(hard_flux_stats["sample_weights"])),
                    "max": float(np.max(hard_flux_stats["sample_weights"])),
                    "weighted_samples": int(np.sum(hard_flux_stats["sample_weights"] > 1.0)),
                },
            },
        }
